...
public class CPacket : MonoBehaviour
{
        ...
        public void Push(Vector3 data)
        {
	        byte[] temp_buffer = new byte[12];
	        Buffer.BlockCopy(BitConverter.GetBytes(data.x),0,temp_buffer,0*sizeof(float),sizeof(float));
	        Buffer.BlockCopy(BitConverter.GetBytes(data.x),0,temp_buffer,1*sizeof(float),sizeof(float));
	        Buffer.BlockCopy(BitConverter.GetBytes(data.x),0,temp_buffer,2*sizeof(float),sizeof(float));
            this.position += temp_buffer.Length;
        }
}
